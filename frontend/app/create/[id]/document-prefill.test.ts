...
describe('collectWizardFieldCandidates', () => {
  it('returns AI wizard fields with source document and evidence', () => {
    const records = [
      recordWithUnderstanding('doc-1', 'grundriss.pdf', understanding([
  ...
  it('ignores non-completed documents and empty values', () => {
    const records = [
      {
        ...recordWithUnderstanding('doc-1', 'a.pdf', understanding([
          { field: 'livingArea', value: null, evidence: null },
        ])),
        status: 'failed' as const,
      },
      recordWithUnderstanding('doc-2', 'b.pdf', understanding([
        { field: 'livingArea', value: null, evidence: null },
      ])),
    ];
...
describe('groupCandidatesByField / pickDefault', () => {
  it('preserves every source and prefers the value with evidence', () => {
    const candidates = [
      { field: 'livingArea', value: 999, sourceDocumentId: 'a', sourceFilename: 'a.pdf', evidence: null },
      { field: 'livingArea', value: 125, sourceDocumentId: 'b', sourceFilename: 'b.pdf', evidence: '125 m²' },
    ];
...
describe('computeWizardPrefills', () => {
  it('fills empty wizard fields from AI values as defaults', () => {
    const records = [
      recordWithUnderstanding('doc-1', 'wohnflaechenberechnung.pdf', understanding([
  ...
  it('never overwrites a value the user has already entered', () => {
    const records = [
      recordWithUnderstanding('doc-1', 'expose.pdf', understanding([
        { field: 'livingArea', value: 107, evidence: 'Wohnfläche: 107 m²' },
      ])),
    ];
  ...
  it('keeps conflicting candidates visible while choosing a default', () => {
    const records = [
      recordWithUnderstanding('expose', 'Expose.pdf', understanding([
        { field: 'livingArea', value: 107, evidence: '107 m²' },
      ])),
      recordWithUnderstanding('berechnung', 'Wohnflaechenberechnung.pdf', understanding([
        { field: 'livingArea', value: 106.4, evidence: '106,4 m²' },
      ])),
    ];
...
describe('additional information candidates', () => {
  it('collects document additional information with source and evidence', () => {
    const records = [
      recordWithUnderstanding('grundbuch', 'Grundbuchauszug.pdf', understanding([], [
  ...
  it('groups additional information by key and preserves multiple documents', () => {
    const records = [
      recordWithUnderstanding('a', 'A.pdf', understanding([], [
        { key: 'owners', value: 'Max Mustermann', evidence: 'Eigentümer: Max Mustermann' },
      ])),
      recordWithUnderstanding('b', 'B.pdf', understanding([], [
