// 360 viewer preview — panorama projection.
// Typed port of the former standalone prototype's `panoramaProjection.js`.
//
// Converts world-space window corners into current 360 view coordinates.
// Chain: floor-plan {x,y} -> 3D world (coordinates.ts, windowGeometry.ts)
...
// World point (three.js, Y-up) -> panorama `{ yaw, pitch }` (degrees),
// accounting for camera world position and panorama orientation:
// worldYaw = yawInPanorama + orientation.
export function worldPointToYawPitch(
  point: WorldPoint,
  cameraPos: WorldPoint,
  orientation = 0,
...
export function yawPitchToScreen(yaw: number, pitch: number, view: ViewState): ScreenPoint {
...
export function projectWorldPoint(
  ...
  orientation: number,
  view: ViewState,
): ScreenPoint & { worldYawPitch: { yaw: number; pitch: number } } {
...
export function projectWindowCorners(
  corners: WorldPoint[],
  cameraPos: WorldPoint,
  orientation: number,
  view: ViewState,
