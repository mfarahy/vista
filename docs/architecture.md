...
`job-processor/` is a standalone worker: it connects to NATS, subscribes to `vista.jobs.>`, parses each event (`src/jobs/event.ts`), dispatches to a handler by `jobType` (`src/jobs/dispatcher.ts`, handlers in `src/jobs/handlers/`), and writes status transitions to the same `Job` table. Handlers report progress via a context `update()` and throw to signal failure; the consumer catches per-job errors, marks the job `failed`, and never crashes the worker (malformed events are dropped with a warning). There is intentionally no Redis, Kafka, Temporal, workflow engine, or retry infrastructure yet.

## Document-processing boundary

Document processing is asynchronous and executed by job-processor. `POST /api/properties/:id/documents` (and the per-document analyze endpoint) persist the file bytes to the swappable object storage (`expose-service/src/lib/document-storage.ts`: `local` disk default, `r2` Cloudflare R2 / S3-compatible via the `CLOUDFLARE_*` variables), create the document record in the shared `Document` table (`expose-service/prisma/schema.prisma`, mirrored in job-processor), enqueue a `document-processing` job and publish `vista.jobs.document-processing`, then return the `jobId` immediately.

The `job-processor` `document-processing` handler (`job-processor/src/jobs/handlers/document-processing.ts`) iterates the payload's `documentIds`; expose-service exposes no internal HTTP endpoints for the worker. Instead the worker runs the pipeline locally: for each document it reads the record from the shared `Document` table, downloads the file bytes directly from the configured object storage (R2), runs the OCR and understanding stages (`job-processor/src/lib/document-analysis`, `document-understanding`, `document-processor`), and writes the results (status, `analysisResult`, `understandingResult`, …) back to the same record. The pipeline logic moved into job-processor so expose-service stays a thin director (upload, record, serve); heavy AI work never passes through it. A failed document is logged and contained; the job completes when at least one document succeeded and is marked `failed` only when every document failed.

The `expose-service/.env` file is the Prisma CLI environment and is intentionally ignored by Git. Copy `.env.example` to `expose-service/.env` for the local PostgreSQL service from `docker-compose.yml`; CI must provide `DATABASE_URL` as a secret environment variable. In production both services must share the same `DATABASE_URL` (for the `Job`/`Document` tables) and the same `CLOUDFLARE_*` R2 credentials (for file bytes); job-processor additionally receives `GOOGLE_DOCUMENT_AI_*` and `OPENAI_*` credentials for the pipeline.
