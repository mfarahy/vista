...
Document processing is asynchronous and executed by job-processor. `POST /api/properties/:id/documents` (and the per-document analyze endpoint) persist file bytes to the swappable document storage (`expose-service/src/lib/document-storage.ts`: `local` disk default, `r2` Cloudflare R2 / S3-compatible via the `CLOUDFLARE_*` variables), create the document records, enqueue a `document-processing` job and publish `vista.jobs.document-processing`, then return the `jobId` immediately.

The `job-processor` `document-processing` handler (`job-processor/src/jobs/handlers/document-processing.ts`) iterates the payload's `documentIds`, and per document invokes the two existing pipeline stages over HTTP (`POST /api/internal/documents/:id/ocr` and `/understand`, `expose-service/src/routes/internal.ts`), reporting `currentStep`/`progress`/`message` between them. The internal endpoints reuse the dependency-injected pipeline (`expose-service/src/lib/document-understanding/pipeline.ts`, refactored into `runOcrStep`/`runUnderstandStep` while keeping `runDocumentPipeline` behaviour exactly), the Google Document AI OCR provider, and the OpenAI understanding provider. A failed document is logged and contained; the job completes when at least one document succeeded and is marked `failed` only when every document failed. The worker reaches expose-service through its `EXPOSE_SERVICE_URL` env var.
