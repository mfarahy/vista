// Geometry-based 360° window overlay (Phase 7).
//
...
import { WINDOWS } from './floorplan.js'
import {
  windowWidthLabel,
  windowWorldCorners,
...
} from './panoramaProjection.js'

const SVG_NS = 'http://www.w3.org/2000/svg'

export function isWindowDebugMode() {
  try {
...
function createOverlaySvg() {
  ...
  svg.setAttribute('class', 'window-overlay')
  ...
  group.setAttribute('class', 'window-overlay-group')
  group.style.transition = 'opacity 180ms ease'
  group.style.opacity = '0'

  const polygon = document.createElementNS(SVG_NS, 'polygon')
  polygon.setAttribute('class', 'window-overlay-shape')
  group.appendChild(polygon)

  const labelBg = document.createElementNS(SVG_NS, 'rect')
  labelBg.setAttribute('class', 'window-overlay-label-bg')
  labelBg.setAttribute('rx', '10')
  labelBg.setAttribute('ry', '10')
  group.appendChild(labelBg)

  const label = document.createElementNS(SVG_NS, 'text')
  label.setAttribute('class', 'window-overlay-label')
...
function createDebugPanel() {
  ...
  div.setAttribute('class', 'window-overlay-debug')
 ...
 */
export function attachWindowOverlay(viewer, container, panoramaId) {
  ...
  const labelText = windowWidthLabel(win)
