...
app.post("/api/properties/:id/location", async (req, res) => {
    ...
    try {
        ...
        if (!intelligence)
            return res.status(422).json({ error: "Location could not be resolved." });
...
app.post("/api/properties/:id/location/refresh", async (req, res) => {
    ...
    try {
        ...
        if (!result.intelligence)
            return res.status(422).json({ error: result.error || "Location could not be resolved." });
...
app.post("/api/demo", async (_req, res) => {
    ...
    for (let index = 1; index <= 6; index += 1)
        await addImage(property.id, { url: `/demo/room-${index}.svg`, fileName: `demo-room-${index}.svg`, mimeType: "image/svg+xml", size: 0, sequence: index - 1, isCover: index === 1 });
