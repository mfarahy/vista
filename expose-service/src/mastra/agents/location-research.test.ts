...
import { buildLocationResearchQueries, researchLocation, validateLocationResearch, type LocationResearchProvider } from "./location-research-agent.js";
...
const input = {
  ...
  latitude: 52.48,
  longitude: 13.43,
  locationIntelligence: { coordinates: { latitude: 52.48, longitude: 13.43 }, facilities: { transport: [{ name: "Station", category: "subway", distanceMeters: 350 }] } },
};
...
function provider(counter: { searches: number; extracts: number }): LocationResearchProvider {
  return {
    async search({ query }) {
      counter.searches += 1;
      return { results: [{ title: `Quelle für ${query}`, url: "https://www.berlin.de/bezirk/", content: "Der Bezirk beschreibt das recherchierte Umfeld mit öffentlichen Informationen.", score: 0.9 }, { title: "Duplikat", url: "https://www.berlin.de/bezirk/", content: "Doppelte Quelle mit gleichem Inhalt.", score: 0.4 }] };
    },
    async extract({ urls }) {
      counter.extracts += 1;
      return { results: urls.map((url) => ({ url, rawContent: "Der offizielle Inhalt bestätigt den lokalen Sachverhalt." })) };
    },
  };
}
...
describe("location research", () => {
  it("builds focused, privacy-preserving German-area queries", () => {
    ...
    assert.ok(queries.filter((item) => item.category !== "makrolage").every((item) => item.query.includes("Reuterkiez")));
    assert.ok(queries.every((item) => !item.query.includes("Weserstraße") && !item.query.includes("42")));
  });
  ...
  it("preserves and deduplicates source URLs and keeps deterministic data separate", async () => {
    ...
    assert.deepEqual(input.locationIntelligence?.coordinates, { latitude: 52.48, longitude: 13.43 });
    assert.ok(research.infrastructure.transport.every((claim) => !claim.statement.match(/\b\d+\s*m\b/)));
