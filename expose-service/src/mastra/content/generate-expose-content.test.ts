...
function referenceProperty() {
  ...
  data.location = {
    ...
    neighborhood: null,
    description: 'Ruhige Wohnlage im Süden Berlins.',
  };
  data.pricing = {
    purchasePrice: 499000,
    rentPrice: null,
    additionalCosts: null,
    buyerCommission: '3,57 % inkl. ges. MwSt.',
  ...
  data.energy = {
    ...
    primaryEnergySource: 'oil',
    finalEnergyDemand: 250.2,
  ...
  data.rooms = [
    {
      ...
      name: 'Wohnzimmer',
      area: 31,
      description: 'Wohnbereich mit Zugang zum Garten.',
      features: [],
      floor: 'Erdgeschoss',
    },
    {
  ...
  data.outdoorAreas = [
    { type: 'garden', area: null, orientation: null, description: 'Garten am Haus.' },
  ];
  data.images = [
    {
      assetId: 'hero-1',
...
describe('generateExposeContent', () => {
  it('creates a complete factual German content structure from canonical data', async () => {
    ...
    assert.deepEqual(
      content.overview.energy?.facts.map((item) => item.label),
      ['Energieausweis', 'Bj. lt. Energieausweis', 'Endenergiebedarf', 'Energieeffizienzklasse'],
    );
    assert.equal(
      content.overview.facts.find((item) => item.label === 'Hauptenergieträger')?.value,
      'Öl',
    );
    ...
    assert.ok(
      content.propertyDescription?.paragraphs.length &&
        content.propertyDescription.paragraphs.length >= 4,
    );
  ...
  it('rejects references to unknown assets', async () => {
    ...
    const content = await generateExposeContent(property);
    assert.throws(() =>
      validateExposeContentReferences(property, {
        ...content,
        cover: { ...content.cover, heroImage: { assetId: 'missing', caption: 'Hausansicht' } },
      }),
    );
  });
});
