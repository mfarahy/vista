...
const energyLabels: Record<string, string> = {
  needs_based: 'bedarfsorientiert',
  consumption_based: 'verbrauchsorientiert',
...
function area(value: number | null | undefined) {
  return has(value) ? `${decimal.format(value)} m²` : '';
}
...
function label(value: string) {
  return propertyTypeLabels[value] || value.replaceAll('_', ' ');
}
function roomLabel(room: PropertyExposeData['rooms'][number]) {
  return clean(room.name) || roomTypeLabels[room.type] || 'Raum';
}
...
function meaningfulLocation(data: PropertyExposeData) {
  return (
    meaningfulAddress(data.location.address) ||
    Boolean(
      clean(data.location.district) ||
      clean(data.location.neighborhood) ||
      clean(data.location.description),
    )
  );
}
...
function facts(data: PropertyExposeData) {
  ...
  return [
    fact(
      'Objektart',
      data.basicInformation.propertyType && label(data.basicInformation.propertyType),
    ),
    ...
    fact(
      ...
      details.yearBuilt || details.completionYear
        ? `ca. ${details.yearBuilt || details.completionYear}`
        : null,
    ),
    fact(
      'Hauptenergieträger',
      data.energy?.primaryEnergySource ? energyLabels[data.energy.primaryEnergySource] : null,
    ),
    fact(
      'Kaufpreis',
...
function energyFacts(data: PropertyExposeData) {
  ...
  const value = [
    ...
    fact(
      'Endenergiebedarf',
      energy.finalEnergyDemand != null
        ? `${decimal.format(energy.finalEnergyDemand)} kWh/(m²·a)`
        : null,
    ),
    fact(
      'Endenergieverbrauch',
      energy.finalEnergyConsumption != null
        ? `${decimal.format(energy.finalEnergyConsumption)} kWh/(m²·a)`
        : null,
    ),
...
function sourceSentences(data: PropertyExposeData) {
  ...
  const location = [
    clean(data.location.address.city),
    clean(data.location.district || data.location.neighborhood),
  ]
    .filter(Boolean)
    .join(', ');
  const sentences = [`Das ${type} befindet sich${location ? ` in ${location}` : ''}.`];
  if (details.livingArea != null || details.rooms != null)
    sentences.push(
      `Die Immobilie verfügt${details.livingArea != null ? ` über ca. ${area(details.livingArea)} Wohnfläche` : ''}${details.livingArea != null && details.rooms != null ? ' und' : ''}${details.rooms != null ? ` ${details.rooms} Zimmer` : ''}.`,
    );
...
export async function generateCoverTitle(data: PropertyExposeData) {
  return (
    clean(data.basicInformation.title) ||
    `${label(data.basicInformation.propertySubtype || data.basicInformation.propertyType)}${data.location.address.city ? ` in ${data.location.address.city}` : ''}`
  );
}

export async function generatePropertySummary(data: PropertyExposeData) {
  const fallback = sourceSentences(data).slice(0, 2).join(' ');
  return generateSectionText(
    'Einleitung',
    {
      type: data.basicInformation.propertyType,
      subtype: data.basicInformation.propertySubtype,
...
export async function generatePropertyDescription(data: PropertyExposeData) {
  const paragraphs: Array<{ heading: string; text: string }> = [
    { heading: 'Einleitung', text: await generatePropertySummary(data) },
  ];
  const layout = data.rooms.length
    ? `Das Raumprogramm umfasst ${data.rooms.map(roomLabel).join(', ')}.`
    : '';
  ...
  if (living.length) {
    const fallback = living
      .map(
        (room) =>
          `${roomLabel(room)}${room.area != null ? ` mit ca. ${area(room.area)}` : ''}${clean(room.description) ? `: ${clean(room.description)}` : '.'}`,
      )
      .join(' ');
    paragraphs.push({
      heading: 'Wohnbereiche',
      text: await generateSectionText('Wohnbereiche', living, fallback, 3000),
    });
  }
  const privateRooms = data.rooms.filter((room) =>
    ['bedroom', 'child_room', 'office'].includes(room.type),
  );
  if (privateRooms.length) {
    const fallback = privateRooms
      .map(
        (room) =>
          `${roomLabel(room)}${room.area != null ? ` mit ca. ${area(room.area)}` : ''}${clean(room.description) ? `: ${clean(room.description)}` : '.'}`,
      )
      .join(' ');
    paragraphs.push({
      heading: 'Private Räume',
      text: await generateSectionText('Private Räume', privateRooms, fallback, 3000),
    });
  }
  const wetRooms = data.rooms.filter((room) =>
    ['kitchen', 'bathroom', 'guest_wc'].includes(room.type),
  );
  if (wetRooms.length) {
    const fallback = wetRooms
      .map(
        (room) =>
          `${roomLabel(room)}${room.area != null ? ` mit ca. ${area(room.area)}` : ''}${room.features.length ? `, ausgestattet mit ${room.features.join(', ')}` : ''}${clean(room.description) ? `: ${clean(room.description)}` : '.'}`,
      )
      .join(' ');
    paragraphs.push({
      heading: 'Küche und Bäder',
      text: await generateSectionText('Küche und Bäder', wetRooms, fallback, 3000),
    });
  }
  if (data.outdoorAreas.length) {
    const fallback = data.outdoorAreas
      .map(
        (item) =>
          `${outdoorLabels[item.type]}${item.area != null ? ` mit ca. ${area(item.area)}` : ''}${clean(item.description) ? `: ${clean(item.description)}` : '.'}`,
      )
      .join(' ');
    paragraphs.push({
      heading: 'Außenbereiche',
      text: await generateSectionText('Außenbereiche', data.outdoorAreas, fallback, 2500),
    });
  }
  const additional = data.rooms.filter((room) =>
    ['basement', 'attic', 'garage', 'hobby_room', 'utility_room'].includes(room.type),
  );
  const additionalTexts = [
    ...
    ...data.equipment
      .filter((item) => ['storage', 'parking'].includes(item.category))
      .map(
        (item) => `${item.name}${clean(item.description) ? `: ${clean(item.description)}` : '.'}`,
      ),
  ];
  if (additionalTexts.length)
    paragraphs.push({ heading: 'Zusätzliche Bereiche', text: additionalTexts.join(' ') });
  const constructionText =
    data.propertyDetails.yearBuilt || data.propertyDetails.completionYear
      ? `Das Baujahr bzw. Fertigstellungsjahr ist ${data.propertyDetails.yearBuilt || data.propertyDetails.completionYear}.`
      : '';
  const energyText = data.energy?.primaryEnergySource
    ? `Als Hauptenergieträger ist ${energyLabels[data.energy.primaryEnergySource]} angegeben.`
    : '';
  if (constructionText || energyText)
    paragraphs.push({
      heading: 'Baujahr und Energie',
      text: [constructionText, energyText].filter(Boolean).join(' '),
    });
  paragraphs.push({
    heading: 'Zusammenfassung',
    text: sourceSentences(data)
...
export async function generateRoomDescriptions(data: PropertyExposeData) {
  return Promise.all(
    data.rooms.map(async (room, index) => ({
      ...
      name: roomLabel(room),
      ...(room.area != null ? { area: area(room.area) } : {}),
      description: await generateSectionText(
        'Raumprogramm',
        room,
        clean(room.description) ||
          `${roomLabel(room)} ist als Teil des angegebenen Raumprogramms vorhanden${room.floor ? ` und liegt auf ${room.floor}` : ''}.`,
        2000,
      ),
    })),
  );
...
export async function generateEquipmentDescription(data: PropertyExposeData) {
  const facts = [
    ...data.equipment.map((item) => ({
      label: item.name,
      value: clean(item.description) || 'vorhanden',
    })),
    ...data.outdoorAreas.map((item) => ({
      label: outdoorLabels[item.type],
      value: item.area != null ? area(item.area) : 'vorhanden',
    })),
    ...[
      data.propertyDetails.garageCount != null
        ? { label: 'Garage', value: String(data.propertyDetails.garageCount) }
        : null,
      data.propertyDetails.parkingSpaceCount != null
        ? { label: 'Stellplätze', value: String(data.propertyDetails.parkingSpaceCount) }
        : null,
    ].filter((item): item is { label: string; value: string } => item !== null),
  ];
  ...
  return {
    facts,
    description: await generateSectionText('Ausstattung im Überblick', facts, fallback, 4000),
  };
}

export async function generateLocationDescription(data: PropertyExposeData) {
  const location = data.location;
  const name = [clean(location.address.city), clean(location.district || location.neighborhood)]
    .filter(Boolean)
    .join(', ');
  const fallback =
    clean(location.description) ||
    (name
      ? `Die Immobilie liegt in ${name}.`
      : 'Zur Lage liegen derzeit nur die angegebenen Adressdaten vor.');
  const description = await generateSectionText(
    'Lage',
    {
  ...
  return {
    description,
    ...(clean(location.district) ? { district: clean(location.district) } : {}),
    ...(clean(location.neighborhood) ? { neighborhood: clean(location.neighborhood) } : {}),
    ...(location.intelligence ? { intelligence: location.intelligence } : {}),
    ...(location.research ? { research: location.research } : {}),
  };
}
...
export async function generateOtherInformation(data: PropertyExposeData) {
  ...
  const items = [
    fact('Rechtliche Hinweise', info.legalNotes),
    fact('Hinweise des Verkäufers', info.sellerNotes),
    fact('Provisionshinweise', info.commissionNotes),
    fact('Verfügbarkeit', info.availability),
  ].filter((item): item is { label: string; value: string } => item !== null);
...
export async function generateAdditionalInformation(data: PropertyExposeData) {
  const items = [
    fact('Zusätzliche Angaben', data.additionalInformation.additionalInformation),
    fact('Parken', data.parking?.description),
  ].filter((item): item is { label: string; value: string } => item !== null);
  return items.length ? { items } : undefined;
}
...
export async function generateImageCaptions(images: ExposeImage[]) {
  return Promise.all(
    images.map(async (image) => ({
      ...
      caption: await generateSectionText(
        'Bildunterschrift',
        {
...
function imageSections(data: PropertyExposeData) {
  ...
  return [...grouped.entries()].map(([key, sectionImages]) => ({
    category: sectionImages[0].category,
    label:
      key === sectionImages[0].category
        ? imageCategoryLabels[sectionImages[0].category]
        : key.replaceAll('_', ' '),
    images: sectionImages.map((image) => ({
      assetId: image.assetId,
...
function planSections(data: PropertyExposeData) {
  return data.floorPlans.length
    ? [
        {
          title: 'Grundrisse / Pläne',
          images: data.floorPlans.map((image) => ({
            assetId: image.assetId,
            caption: imageCaption(image),
          })),
        },
...
function mapSections(data: PropertyExposeData) {
  return data.maps.length
    ? [
        {
          title: 'Lageplan / Makrolage',
          images: data.maps.map((image) => ({
            assetId: image.assetId,
            caption: imageCaption(image),
          })),
        },
...
function vistaSection(data: PropertyExposeData) {
  ...
  return {
    ...
    description:
      clean(branding.description) ||
      `${branding.companyName} begleitet den Weg von den ersten Informationen bis zum Kaufabschluss.`,
    steps: branding.processSteps.length
      ? branding.processSteps
      : [
          'Das Exposé',
          'Interesse und Vorstellung als neues Zuhause',
          'Finanzierung',
          'Besichtigung und Entscheidung',
          'Notar und Kaufabschluss',
        ],
...
export async function generateVistaClosingContent(data: PropertyExposeData) {
  return vistaSection(data);
}
...
export async function generateExposeContent(data: PropertyExposeData): Promise<ExposeContent> {
  ...
  const cover: ExposeContent['cover'] = {
    ...
    clean(data.location.address.city) ||
    clean(data.basicInformation.address.city)
      ? {
          location:
            addressText(data.location.address) ||
            clean(data.location.address.city) ||
  ...
  const hero =
    data.images
      .filter((image) => image.category !== 'floor_plan')
      .find((image) => image.isHeroCandidate) ||
    data.images.find((image) => image.category === 'exterior');
  if (hero) cover.heroImage = { assetId: hero.assetId, caption: imageCaption(hero) };
  if (data.pricing.purchasePrice != null)
    cover.purchasePrice = euro.format(data.pricing.purchasePrice);
  if (data.propertyDetails.livingArea != null)
    cover.livingArea = `ca. ${area(data.propertyDetails.livingArea)}`;
  ...
  if (meaningfulAddress(data.basicInformation.address))
    content.objectInformation = { address: data.basicInformation.address };
