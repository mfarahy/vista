...
function imageCaption(asset: Asset, fallback?: string) {
  ...
  if (asset.category === 'interior')
    return 'room' in asset ? clean(asset.room) || 'Innenansicht' : 'Innenansicht';
...
function allAssets(property: Property) {
  ...
  for (const asset of [
    ...property.images,
    ...(property.exposeData?.images || []),
    ...(property.exposeData?.floorPlans || []),
    ...(property.exposeData?.maps || []),
  ]) {
...
async function resolveReference(
  reference: StructuredExposeImageReference,
...
function factSection(title: string, facts: StructuredExposeFact[], description?: string) {
  return `<section class="page section-page"><div class="section-head"><p class="kicker">VISTA IMMOBILIEN-EXPOSÉ</p><h2>${esc(title)}</h2></div>${factGrid(facts)}${
    description
      ? `<div class="prose compact">${paragraphs(description)
...
async function galleryPages(
  sections: NonNullable<StructuredExposeContent['imageSections']>,
  assets: Map<string, Asset>,
) {
  const refs = sections
    .filter((section) => section.category !== 'floor_plan')
    .flatMap((section) => section.images.map((image) => ({ ...image, section: section.label })));
  const resolved = (
    await Promise.all(
      refs.map(async (ref) => ({ ref, image: await resolveReference(ref, assets) })),
    )
  ).filter(
    (
      item,
      ...
      image: NonNullable<Awaited<ReturnType<typeof resolveReference>>>;
    } => Boolean(item.image),
  );
  const pages: string[] = [];
...
async function planPages(
  title: string,
  refs: StructuredExposeImageReference[],
  assets: Map<string, Asset>,
  kind: 'plan' | 'map',
) {
  const resolved = (
  ...
  return resolved.map(
    ({ image }) =>
      `<section class="page plan-page"><div class="section-head"><p class="kicker">${kind === 'map' ? 'LAGE' : 'GRUNDRISSE'}</p><h2>${esc(title)}</h2></div><figure class="plan"><img src="${image.src}" alt="${esc(image.caption)}"/><figcaption>${esc(image.caption)}</figcaption></figure></section>`,
  );
}
...
function researchBlock(research: NonNullable<StructuredExposeContent['location']>['research']) {
  if (!research) return '';
  const claims = [
    ...research.mikrolage.claims,
    ...research.makrolage.claims,
    ...research.infrastructure.transport,
    ...research.infrastructure.education,
    ...research.infrastructure.shopping,
    ...research.infrastructure.recreation,
  ].slice(0, 8);
  if (!claims.length) return '';
  const sourceDomains = [...new Set(research.sources.map((source) => source.domain))].slice(0, 8);
  return `${subsectionHead('Recherche zur Lage', 'QUELLENGESTÜTZTER KONTEXT')}<div class="research-list">${claims.map((claim) => `<article><p>${esc(claim.statement)}</p><small>${esc(claim.factType === 'hard_fact' ? 'Belegter Fakt' : 'Kontextuelle Einordnung')} · ${esc(claim.sources.map((source) => source.domain).join(', '))}</small></article>`).join('')}</div><p class="location-source">Recherche vom ${esc(new Date(research.researchedAt).toLocaleDateString('de-DE'))}; Quellen: ${esc(sourceDomains.join(', '))}</p>`;
}

function locationPage(location: NonNullable<StructuredExposeContent['location']>) {
  const intelligence = location.intelligence;
  if (!intelligence) return '';
  const groups: Array<[string, keyof typeof intelligence.facilities]> = [
  ...
  const facilities = groups
    .flatMap(([label, key]) => {
      const place = intelligence.facilities[key][0];
      return place
        ? [
            `<div class="location-row"><span>${esc(label)}</span><strong>${esc(place.name)}</strong><b>${esc(locationDistance(place.distanceMeters))}</b></div>`,
          ]
        : [];
    })
    .join('');
  const map = intelligence.mapAsset?.url
    ? `<img class="location-map" src="${esc(intelligence.mapAsset.url)}" alt="${esc(intelligence.mapAsset.caption)}"/>`
    : `<div class="location-map location-map-empty">Keine Kartenansicht verfügbar.</div>`;
  return `<section class="page location-page"><div class="section-head"><p class="kicker">LAGE &amp; UMGEBUNG</p><h2>Lage</h2></div><div class="location-map-wrap">${map}</div><div class="location-summary"><h3>Umgebung auf einen Blick</h3><p>${esc(intelligence.summary)}</p></div><div class="location-list">${facilities || `<p class="location-empty">Keine relevanten Einrichtungen in der ausgewählten Umgebung gefunden.</p>`}</div><p class="location-source">Entfernungen sind Luftlinien und keine Wegstrecken.</p>${researchBlock(location.research)}</section>`;
}

function closingPage(content: StructuredExposeContent['vistaSection']) {
  return `<section class="page vista-page"><div class="vista-mark">V</div><p class="kicker">${esc(content.subtitle)}</p><h2>${esc(content.heading)}</h2><div class="prose"><p>${esc(content.description)}</p></div><ol class="process">${content.steps
    .slice(0, 5)
    .map((step, index) => `<li><b>0${index + 1}</b><span>${esc(step)}</span></li>`)
    .join('')}</ol><div class="contact-line">${[content.website, content.email, content.phone]
    .filter(Boolean)
    .map((item) => esc(item))
    .join(' · ')}</div></section>`;
}
...
function heroCandidates(property: Property, content: StructuredExposeContent) {
  ...
  const ordered = [...unique.values()].filter(
    (asset) => asset.category !== 'floor_plan' && asset.category !== 'document',
  );
  const priority = (asset: Asset) => {
    if (asset.isHeroCandidate || ('isCover' in asset && asset.isCover)) return 0;
    if (
      asset.category === 'exterior' &&
      ['front', 'entrance', 'garden', 'terrace'].some((item) =>
        clean(asset.subcategory).toLowerCase().includes(item),
      )
    )
      return 1;
    if (asset.category === 'exterior') return 2;
    if (asset.category === 'interior' && clean(asset.subcategory).toLowerCase().includes('living'))
      return 3;
...
function legacyContent(property: Property, content: ExposeContent): StructuredExposeContent {
  ...
  const address: Address = property.exposeData?.basicInformation.address || {
  ...
  const images = property.images
    ...
    .map((image) => ({ assetId: image.assetId || image.id, caption: imageCaption(image) }));
  const facts = [
    ...(content.factualSnapshot || []).map((item) => {
      const [value, ...labelParts] = item.split(' ');
      return { label: labelParts.join(' ') || 'Angabe', value };
    }),
    ...(property.livingArea
      ? [{ label: 'Wohnfläche', value: `ca. ${property.livingArea} m²` }]
      : []),
    ...(content.factualSnapshot.length || property.livingArea
      ? []
    ...
    ? [
        {
          label: 'Objektart',
          value:
            source.basicInformation.propertyType === 'house'
              ? 'Haus'
              : source.basicInformation.propertyType,
        },
        source.basicInformation.propertySubtype
          ? { label: 'Objekttyp', value: source.basicInformation.propertySubtype }
          : null,
        source.propertyDetails.livingArea != null
          ? { label: 'Wohnfläche', value: `ca. ${source.propertyDetails.livingArea} m²` }
          : null,
        source.propertyDetails.plotArea != null
          ? { label: 'Grundstücksfläche', value: `${source.propertyDetails.plotArea} m²` }
          : null,
        source.propertyDetails.rooms != null
          ? { label: 'Zimmer', value: String(source.propertyDetails.rooms) }
          : null,
        source.propertyDetails.bathrooms != null
          ? { label: 'Badezimmer', value: String(source.propertyDetails.bathrooms) }
          : null,
        source.propertyDetails.garageCount != null
          ? { label: 'Anzahl Garagen', value: String(source.propertyDetails.garageCount) }
          : null,
        source.propertyDetails.parkingSpaceCount != null
          ? { label: 'Anzahl Stellplätze', value: String(source.propertyDetails.parkingSpaceCount) }
          : null,
        source.propertyDetails.yearBuilt || source.propertyDetails.completionYear
          ? {
              label: 'Baujahr/Fertigstellung',
              value: `ca. ${source.propertyDetails.yearBuilt || source.propertyDetails.completionYear}`,
            }
          : null,
        source.energy?.primaryEnergySource
          ? {
              label: 'Hauptenergieträger',
              value:
                source.energy.primaryEnergySource === 'oil'
                  ? 'Öl'
                  : source.energy.primaryEnergySource,
            }
          : null,
        source.pricing.purchasePrice != null
          ? { label: 'Kaufpreis', value: money(source.pricing.purchasePrice) }
          : null,
        source.pricing.buyerCommission
          ? { label: 'Käuferprovision', value: source.pricing.buyerCommission }
          : null,
      ].filter((item): item is { label: string; value: string } => Boolean(item))
    : facts;
  const energyFacts = source?.energy
    ? [
        source.energy.certificateType
          ? {
              label: 'Energieausweis',
              value:
                source.energy.certificateType === 'needs_based'
                  ? 'bedarfsorientiert'
                  : source.energy.certificateType === 'consumption_based'
                    ? 'verbrauchsorientiert'
                    : source.energy.certificateType,
            }
          : null,
        source.energy.yearOfConstruction
          ? { label: 'Bj. lt. Energieausweis', value: String(source.energy.yearOfConstruction) }
          : null,
        source.energy.finalEnergyDemand != null
          ? {
              label: 'Endenergiebedarf',
              value: `${source.energy.finalEnergyDemand.toFixed(2).replace('.', ',')} kWh/(m²·a)`,
            }
          : null,
        source.energy.finalEnergyConsumption != null
          ? {
              label: 'Endenergieverbrauch',
              value: `${source.energy.finalEnergyConsumption.toFixed(2).replace('.', ',')} kWh/(m²·a)`,
            }
          : null,
        source.energy.efficiencyClass
          ? { label: 'Energieeffizienzklasse', value: source.energy.efficiencyClass }
          : null,
      ].filter((item): item is { label: string; value: string } => Boolean(item))
    : undefined;
  const sourceImages =
    source?.images
      .filter((image) => image.category === 'exterior' || image.category === 'interior')
      .map((image) => ({ assetId: image.assetId, caption: imageCaption(image) })) || images;
  const planImages =
    source?.floorPlans.map((image) => ({ assetId: image.assetId, caption: imageCaption(image) })) ||
    [];
  const mapImages =
    source?.maps.map((image) => ({ assetId: image.assetId, caption: imageCaption(image) })) || [];
  return {
    version: 2,
    cover: {
      title: source?.basicInformation.title || content.title,
      location: [address.city, address.district].filter(Boolean).join(' · '),
      purchasePrice: money(
        source?.pricing.purchasePrice ??
          (property.transactionType === 'sale' ? property.askingPrice : property.coldRent),
      ),
      livingArea: source?.propertyDetails.livingArea
        ? `ca. ${source.propertyDetails.livingArea} m²`
        : property.livingArea
          ? `ca. ${property.livingArea} m²`
          : undefined,
      rooms: source?.propertyDetails.rooms
        ? String(source.propertyDetails.rooms)
        : property.rooms
          ? String(property.rooms)
          : undefined,
      heroImage: sourceImages[0] || images[0],
    },
    overview: {
      facts: structuredFacts,
      ...(energyFacts?.length ? { energy: { facts: energyFacts } } : {}),
    },
    objectInformation: { address },
    propertyDescription: {
      paragraphs: [{ heading: 'Objektbeschreibung', text: content.mainDescription }],
    },
    roomProgram: content.roomDescriptions.map((room) => ({
      roomId: room.roomId,
      name: room.name,
    ...
    equipment: source?.equipment.length
      ? {
          facts: source.equipment.map((item) => ({
            label: item.name,
            value: item.description || 'vorhanden',
          })),
        }
      : content.highlights.length
        ? { facts: content.highlights.map((item) => ({ label: 'Highlight', value: item })) }
        : undefined,
    location:
      source?.location.description || content.locationDescription
        ? {
            description: source?.location.description || content.locationDescription,
            district: source?.location.district || undefined,
            neighborhood: source?.location.neighborhood || undefined,
            ...(source?.location.intelligence
              ? { intelligence: source.location.intelligence }
              : {}),
            ...(source?.location.research ? { research: source.location.research } : {}),
          }
        : undefined,
    imageSections: sourceImages.length
      ? [{ category: 'interior', label: 'Objektbilder', images: sourceImages }]
      : undefined,
    planSections: planImages.length ? [{ title: 'Grundrisse', images: planImages }] : undefined,
    mapSections: mapImages.length
      ? [{ title: 'Lageplan / Makrolage', images: mapImages }]
      : undefined,
    agentSection: source?.agent,
    vistaSection: {
      heading: '5 Schritte zur Wunschimmobilie',
      subtitle: source?.systemBranding.companyName || 'Vista',
      description:
        source?.systemBranding.description ||
        content.targetAudience ||
        'Vista begleitet Sie auf dem Weg zu Ihrer Wunschimmobilie.',
      steps: source?.systemBranding.processSteps?.length
        ? source.systemBranding.processSteps
        : ['Exposé', 'Interesse', 'Finanzierung', 'Besichtigung', 'Kaufabschluss'],
      website: source?.systemBranding.website || undefined,
      email: source?.systemBranding.email || undefined,
      phone: source?.systemBranding.phone || undefined,
    },
  };
}

export async function exposeHTML(property: Property, rawContent: RenderContent) {
  const content = isStructuredContent(rawContent)
    ? rawContent
    : legacyContent(property, rawContent);
  ...
  const hero =
    (
      await Promise.all(
        heroCandidates(property, data).map((reference) => resolveReference(reference, assets)),
      )
    ).find(Boolean) || null;
  const sections: string[] = [];
  sections.push(
    `<section class="page cover"><div class="cover-hero" ${hero ? `style="background-image:url('${hero.src}')"` : ''}><span class="cover-brand">VISTA</span></div><div class="cover-copy"><p class="kicker">IMMOBILIEN-EXPOSÉ</p><h1>${esc(data.cover.title)}</h1><p class="cover-location">${esc(data.cover.location || '')}</p><div class="cover-facts">${[
      ['Kaufpreis', data.cover.purchasePrice],
      ['Wohnfläche', data.cover.livingArea],
      ['Zimmer', data.cover.rooms],
    ]
      .filter(([, value]) => value)
      .map(
        ([label, value]) => `<div><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`,
      )
      .join('')}</div></div></section>`,
  );
  sections.push(
    `<section class="page overview-page"><div class="section-head"><p class="kicker">01</p><h2>Auf einen Blick</h2></div>${factGrid(data.overview.facts)}${data.overview.energy ? `<div class="energy"><h3>Energieangaben</h3>${factGrid(data.overview.energy.facts)}${efficiencyClass(data.overview.energy.facts.find((fact) => fact.label.toLowerCase().includes('effizienzklasse'))?.value)}</div>` : ''}${
      data.objectInformation
        ? `<div class="overview-address">${subsectionHead('Objektadresse', 'OBJEKTINFORMATIONEN')}<div class="address-card">${addressLines(
            data.objectInformation.address,
          )
            .map(
              (line, index) =>
                `<p class="${index === 0 ? 'address-primary' : ''}">${esc(line)}</p>`,
            )
            .join('')}</div></div>`
        : ''
    }</section>`,
  );
  if (data.propertyDescription || data.roomProgram?.length)
    sections.push(
      `<section class="page section-page content-page"><div class="section-head"><p class="kicker">OBJEKTINFORMATIONEN</p><h2>Objektbeschreibung</h2></div>${
        data.propertyDescription
          ? data.propertyDescription.paragraphs
              .map(
                (paragraph) =>
                  `<article class="copy-block"><h3>${esc(paragraph.heading)}</h3>${paragraphs(
                    paragraph.text,
                  )
                    .map((item) => `<p>${esc(item)}</p>`)
                    .join('')}</article>`,
              ...
              .join('')
          : ''
      }${data.roomProgram?.length ? `${subsectionHead('Räume und Aufteilung', 'RAUMPROGRAMM')}<div class="room-list">${data.roomProgram.map((room) => `<article><div><h3>${esc(room.name)}</h3>${room.area ? `<span>${esc(room.area)}</span>` : ''}</div><p>${esc(room.description)}</p></article>`).join('')}</div>` : ''}</section>`,
    );
  if (data.equipment || data.location)
    sections.push(
      `<section class="page section-page content-page"><div class="section-head"><p class="kicker">AUSSTATTUNG UND LAGE</p><h2>Details zum Objekt</h2></div>${
        data.equipment
          ? `${subsectionHead('Ausstattung im Überblick', 'AUSSTATTUNG')}${factGrid(data.equipment.facts)}${
              data.equipment.description
                ? `<div class="prose compact">${paragraphs(data.equipment.description)
                    .map((item) => `<p>${esc(item)}</p>`)
                    .join('')}</div>`
                : ''
            }`
        ...
        data.location
          ? `${subsectionHead('Lage', 'LAGE')}${[data.location.district, data.location.neighborhood].filter(Boolean).length ? `<p class="location-meta">${esc([data.location.district, data.location.neighborhood].filter(Boolean).join(' · '))}</p>` : ''}<div class="prose">${paragraphs(
              data.location.description,
            )
              .map((item) => `<p>${esc(item)}</p>`)
              .join(
                '',
              )}</div>${data.location.intelligence ? '' : researchBlock(data.location.research)}`
          : ''
      }</section>`,
    );
  if (data.location?.intelligence) sections.push(locationPage(data.location));
  if (data.otherInformation)
    sections.push(factSection('Sonstige Angaben', data.otherInformation.items));
  if (data.additionalInformation)
    sections.push(factSection('Zusatzinformationen', data.additionalInformation.items));
  sections.push(...(await galleryPages(data.imageSections || [], assets)));
  for (const plan of data.planSections || [])
    sections.push(...(await planPages(plan.title, plan.images, assets, 'plan')));
  for (const map of data.mapSections || [])
    sections.push(...(await planPages(map.title, map.images, assets, 'map')));
  if (data.agentSection) {
    const agent = data.agentSection;
    sections.push(
      `<section class="page contact-page"><div class="section-head"><p class="kicker">KONTAKT</p><h2>Ihr Ansprechpartner</h2></div><div class="contact-card">${agent.name ? `<h3>${esc(agent.name)}</h3>` : ''}${agent.company ? `<p>${esc(agent.company)}</p>` : ''}${addressLines(
        agent.address,
      )
        .map((line) => `<p>${esc(line)}</p>`)
        .join('')}${[agent.phone, agent.email, agent.website]
        .filter(Boolean)
        .map((line) => `<p>${esc(line)}</p>`)
        .join('')}</div></section>`,
    );
