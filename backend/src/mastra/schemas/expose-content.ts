...
const forbiddenVisibleText = /\[\s*(?:insert|address|price|title|value)|\{\{|\}\}|\b(?:undefined|null|n\/a)\b|(?:screenshot|image|img|photo|scan)[_-]?\d*\.(?:png|jpe?g|webp|pdf)\b/i;
...
function inspectText(value: unknown, path: string, issues: string[]) {
  ...
  else if (Array.isArray(value)) value.forEach((item, index) => inspectText(item, `${path}[${index}]`, issues));
  else if (value && typeof value === "object") Object.entries(value).forEach(([key, item]) => inspectText(item, `${path}.${key}`, issues));
}
...
export function validateExposeContent(value: unknown): ExposeContent {
  ...
  if (issues.length) throw new Error(`Expose content contains forbidden placeholder or filename text at ${issues.join(", ")}`);

  const references = new Set([
    ...(parsed.imageSections ?? []).flatMap((section) => section.images.map((image) => image.assetId)),
    ...(parsed.planSections ?? []).flatMap((section) => section.images.map((image) => image.assetId)),
    ...(parsed.mapSections ?? []).flatMap((section) => section.images.map((image) => image.assetId)),
  ]);
...
export function validateExposeContentReferences(property: {
  rooms: Array<{ id?: string }>;
  images: Array<{ assetId: string }>;
  floorPlans: Array<{ assetId: string }>;
  maps: Array<{ assetId: string }>;
}, content: ExposeContent) {
  const imageIds = new Set([...property.images, ...property.floorPlans, ...property.maps].map((image) => image.assetId));
  const references = [
  ...
  if (references.some((image) => !imageIds.has(image.assetId))) throw new Error("Expose content references an unknown image asset");
  const roomIds = new Set(property.rooms.map((room) => room.id).filter((id): id is string => Boolean(id)));
  if ((content.roomProgram ?? []).some((room) => !roomIds.has(room.roomId))) throw new Error("Expose content references an unknown room");
  return content;
}
