...
import { prepareExposeDataOutputSchema, generateExposeContentOutputSchema, validateExposeContentReferences } from "../schemas/index.js";
...
export const propertyExposeAgent = new Agent({
  ...
  instructions: "Coordinate section-based German exposé generation from validated PropertyExposeData. Use only supplied facts, omit unavailable sections, and never invent property or location information.",
...
export async function generatePropertyExposeContent(property: PropertyExposeData) {
  ...
  return generateExposeContentOutputSchema.parse({ property: prepared.property, content: validateExposeContentReferences(prepared.property, content) });
}
