...
import { generateExposeContent } from "./lib/ai.js";
dotenv.config();
const app = express();
...
app.get("/ready", async (_req, res) => {
    try {
        ...
        res.json({ status: "ready", service: "vista-backend" });
    }
    catch {
        res.status(503).json({ status: "not-ready", service: "vista-backend" });
    }
});
...
app.listen(port, host, () => {
    console.log(`Vista backend listening on http://${host}:${port}`);
});
